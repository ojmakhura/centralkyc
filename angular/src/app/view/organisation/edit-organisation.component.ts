...
export abstract class EditOrganisationComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    domainsFilterCtrl: FormControl = new FormControl();

    domainsCompare(o1: Set<OrganisationDomain> | any, o2: Set<OrganisationDomain> | any) {
        return false;
    }

    filterDomains() {}

    domainsBackingList: OrganisationDomain[] = [];
    domainsFilteredList: WritableSignal<Set<OrganisationDomain> | any[]> = linkedSignal(() => []);
    domainsChipControl: FormControl = new FormControl([]);

    domainsDisplays: string[] = [
        'name',
        'verified',
    ];
    ...
    ngOnInit() {
        ...
        this.editOrganisationFormValueSubscription = this.editOrganisationForm.valueChanges.subscribe(
            (change: any) => {
...
//            phoneNumbers: this.formBuilder.array(this.editOrganisationVarsForm.phoneNumbers ? this.editOrganisationVarsForm.phoneNumbers : []),
            ...
            physicalAddress: [{value: this.editOrganisationVarsForm.physicalAddress, disabled: false}, [Validators.required, ]],
    ...
    createOrganisationDomainArray(domains?: OrganisationDomain[]): FormArray<FormGroup> {

        let groupArrayControl = this.formBuilder.array<FormGroup>([])

        for(let domain of (domains || [])) {
            groupArrayControl.push(this.createOrganisationDomainGroup(domain))
        }

        return groupArrayControl;

    }

    createOrganisationDomainGroup(domain?: OrganisationDomain): FormGroup {
